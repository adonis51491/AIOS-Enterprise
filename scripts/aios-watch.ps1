...
function Get-Fingerprint {
  ...
  $normalized = ($Text -replace '\d+', '#') -replace '\s+', ' '
  $bytes = [System.Text.Encoding]::UTF8.GetBytes($normalized.Trim())
...
Automatically generated by AIOS v9.1 NightWatch.
"@
...
"@
  ...
  $promptContent = @"
Follow AIOS Enterprise v9.1 NightWatch.
...
- $($SourceFile.Replace($ProjectRoot + "\", "").Replace("\", "/"))
...
function Process-LogFile {
  ...
  try {
    ...
    try {
      ...
      $State["last_positions"][$key] = $stream.Position
    } finally {
      $reader.Dispose()
    }
  } finally {
  ...
  foreach ($line in $lines) {
    ...
    $window.Add($line)
    if ($window.Count -gt 20) { $window.RemoveAt(0) }

    if (Test-IsError -Text $line) {
      $evidenceText = ($window -join [Environment]::NewLine)
      $fingerprint = Get-Fingerprint -Text $evidenceText

      if ($State["fingerprints"].ContainsKey($fingerprint)) {
        continue
      }

      New-AutoBug -SourceFile $Path -EvidenceLines $window.ToArray() -Fingerprint $fingerprint -State $State
      break
    }
  }
}
