...
function Resolve-PathInRoot([string]$RelativePath) {
...
function Get-TaskPath([string]$Id) {
    ...
    if ($Id -notmatch "^[A-Z][A-Z0-9_-]{2,63}$") {
        throw "Invalid TaskId: $Id"
    }
    return Resolve-PathInRoot ".aios-data\tasks\$Id.json"
}
...
function Assert-NoOtherActiveTask([string]$ExceptId) {
    ...
    foreach ($file in Get-ChildItem -LiteralPath $tasksRoot -Filter "*.json" -File) {
        ...
        if ($task.id -ne $ExceptId -and $task.state -notin $TerminalStates) {
            throw "Another non-terminal task exists: $($task.id) [$($task.state)]"
        }
    }
}
...
function Record-Plan {
    ...
    $task.approval.planSha256 = $null
    $task.approval.approvedAt = $null
...
function Approve-Plan {
    $task = Read-Task $TaskId
    if ($task.state -ne "WAITING_APPROVAL") {
        throw "Task must be WAITING_APPROVAL before approval. Current: $($task.state)"
    }
    if ([string]::IsNullOrWhiteSpace($task.plan.sha256)) {
        throw "No recorded plan exists for $TaskId."
    }
    if ($PlanHash.ToLowerInvariant() -ne $task.plan.sha256.ToLowerInvariant()) {
        throw "Approval hash does not match the recorded plan."
    }
    $task.approval.approved = $true
    $task.approval.planSha256 = $task.plan.sha256
    $task.approval.approvedAt = (Get-Date).ToUniversalTime().ToString("o")
...
function Move-TaskState {
    ...
    if ($target -notin @($Transitions[$current])) {
        throw "Illegal transition: $current -> $target"
    }
...
function Render-Status {
    ...
    $lines.Add("# AIOS Task Status")
    $lines.Add("")
    $lines.Add("Generated from `.aios-data/tasks/*.json`. Do not edit task states here.")
    $lines.Add("")
    $tasks = @()
    foreach ($file in Get-ChildItem -LiteralPath $tasksRoot -Filter "*.json" -File | Sort-Object Name) {
        $tasks += Get-Content -LiteralPath $file.FullName -Raw -Encoding UTF8 | ConvertFrom-Json
    }
    foreach ($task in $tasks) {
        $lines.Add("- **$($task.id)** — $($task.title) — `$($task.state)`")
    }
...
function Invoke-Doctor {
    ...
    foreach ($file in Get-ChildItem -LiteralPath $tasksRoot -Filter "*.json" -File) {
        try {
            $task = Get-Content -LiteralPath $file.FullName -Raw -Encoding UTF8 | ConvertFrom-Json
            if ($seen.ContainsKey($task.id)) { $errors.Add("Duplicate task ID: $($task.id)") }
            $seen[$task.id] = $true
            if ($task.state -notin $AllowedStates) { $errors.Add("Unknown state for $($task.id): $($task.state)") }
            if ($task.state -notin $TerminalStates) { $active += $task.id }
            if ($task.approval.approved -and $task.approval.planSha256 -ne $task.plan.sha256) {
                $errors.Add("Stale approval hash for $($task.id)")
            }
            foreach ($evidence in @($task.evidence)) {
                if ([string]::IsNullOrWhiteSpace($evidence.commitSha) -or [string]::IsNullOrWhiteSpace($evidence.command)) {
                    $errors.Add("Incomplete evidence for $($task.id)")
                }
            }
        }
        catch {
            $errors.Add("Unreadable task file $($file.Name): $($_.Exception.Message)")
        }
    }
    if ($active.Count -gt 1) { $errors.Add("More than one non-terminal task exists: $($active -join ', ')") }

    $duplicatePatterns = @(".aios\.aios", "scripts\scripts", "templates\templates", "docs\docs")
    foreach ($pattern in $duplicatePatterns) {
        ...
        if ($null -ne $match) { $errors.Add("Nested duplicate directory detected: $($match.FullName)") }
    }

    if ($errors.Count -gt 0) {
        foreach ($error in $errors) { Write-Host "[FAIL] $error" }
        throw "AIOS v11 doctor found $($errors.Count) issue(s)."
    }
...
switch ($Command) {
    "help" {
        Write-Host "AIOS Enterprise v11 — Governed Engineering Orchestrator"
        Write-Host "Commands:"
        Write-Host "  init"
        Write-Host "  task-create -TaskId ID -Title TITLE"
        Write-Host "  task-status -TaskId ID"
        Write-Host "  plan-record -TaskId ID -PlanPath PATH"
        Write-Host "  approve -TaskId ID -PlanHash SHA256"
        Write-Host "  transition -TaskId ID -To STATE [-Notes TEXT]"
        Write-Host "  evidence-record -TaskId ID -CheckName NAME -CheckCommand COMMAND -ExitCode N -CommitSha SHA -Branch BRANCH"
        Write-Host "  render-status"
        Write-Host "  doctor"
    }
    "init" {
        Ensure-Directory ".aios-data\tasks" | Out-Null
        Ensure-Directory ".aios-data\plans" | Out-Null
        Ensure-Directory ".aios-data\evidence" | Out-Null
        Write-Host "Initialized AIOS v11 project data store."
    }
